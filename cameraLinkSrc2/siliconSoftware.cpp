...
bool siSoftware::isFrameAvailable(void) {
  ...
  if (frame_count == FG_TIMEOUT_ERR) return false;
...
bool siSoftware::getFrame(void *copy_memory, unsigned int *siSoftware_timestamp,
  ...
  if (isFrameAvailable()) {
    ...
    buffnum = Fg_getImageEx(fg, param, frames_to_cpu + 1, camPort, 0,
                            (dma_mem *)ptrMem);
    //            buffnum= Fg_getImageEx(fg, SEL_NEXT_IMAGE,0, camPort,
    //            4,(dma_mem*)ptrMem);
    retrycnt = 0;
    while (dbg_last_imgnum == (buffnum) && retrycnt < 5) {
      int aa;
      aa = 1;
      //            printf("siSoftware::getFrame() - Grabbed same img twice from
      //            grabber-buffnum= %d frames_to_cpu+1=
      //            %d\n",buffnum,frames_to_cpu+1);
      buffnum = Fg_getImageEx(fg, param, frames_to_cpu + 1, camPort, 4,
                              (dma_mem *)ptrMem);
      retrycnt++;
      // trap here
    }
    if (dbg_last_imgnum == buffnum) {
      sprintf(mesx,
              "siSoftware::getFrame() - Grabbed same img twice from "
              "grabber-buffnum= %d frames_to_cpu+1= %d\n",
              buffnum, frames_to_cpu + 1);
      lf.log(mesx);
    }
    ...
    if (buffnum >= 0) {
      ...
      if (image_address != 0) {
        if (copy_memory != 0) memcpy(copy_memory, image_address, nbytes);

        *siSoftware_timestamp = buffnum;
        Fg_getParameter(fg, FG_TIMESTAMP, siSoftware_timestamp, camPort);
        if (is_display) DrawBuffer(dispId0, image_address, buffnum, "");
        if (is_blocking)
      ...
      lf.log(mesx);
      // add jan 2016 so bad buffers can jump out of loops etc.
      return true;
    }
  }
...
bool siSoftware::initialize(int size_x, int size_y, bool is_force_size) {
  ...
  sprintf(mesx, "siSoftware::initialize sizex = %d, sizey=%d \n", size_x,
          size_y);
  lf.log(mesx);
  ...
  if (IdVersion < 0) {
    lf.log(
        "siSoftware::initialize HAP_PCOGLSH version not avaiable- assume "
        "normal FW");
  } else {
    int val;
  ...
  if (is_force_size && !is_edge_globalshut) {
    ...
    // when we config the grabber- we assume we convert raw 8 bit data to 16bit
    // pixels.
    // 10 tap mode assumed...
    // if we have 12 bit data, we have to set is_doub_width to 1.5... a real
    // pain...
    ...
    // we need to mult by 2 for 16 bit images...sizex and sizey are for 16 bit
    // images
    // we assume grabber is set for 8 bit 10 tyap data, with 2 raw pixels for
    // one 16biyt image piuxel..
  ...
  if (is_force_size && is_edge_globalshut) {
    ...
    Idxoff0 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM0_XOffset");
    Idxlen0 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM0_XLength");
    Idyoff0 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM0_YOffset");
    Idylen0 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM0_YLength");

    Idxoff1 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM1_XOffset");
    Idxlen1 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM1_XLength");
    Idyoff1 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM1_YOffset");
    Idylen1 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferReset_DRAM1_YLength");

    Idxoff2 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM0_XOffset");
    Idxlen2 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM0_XLength");
    Idyoff2 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM0_YOffset");
    Idylen2 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM0_YLength");

    Idxoff3 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM1_XOffset");
    Idxlen3 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM1_XLength");
    Idyoff3 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM1_YOffset");
    Idylen3 = Fg_getParameterIdByName(
        fg, "Device1_Process0_ImageBufferData_DRAM1_YLength");

    Idsplitter = Fg_getParameterIdByName(
        fg, "Device1_Process0_DoubleFrameSplit_Splitter_ImageHeight");

    Idenable = Fg_getParameterIdByName(
        fg, "Device1_Process0_EnableSubImage_Enable_Value");
    Idoffset =
        Fg_getParameterIdByName(fg, "Device1_Process0_SubImages_Offset_Value");

    ConstLGShift4_Value_Id = Fg_getParameterIdByName(
        fg, "Device1_Process0_Match16Bit_ConstLGShift4_Value");
    ConstHGShift4_Value_Id = Fg_getParameterIdByName(
        fg, "Device1_Process0_Match16Bit_ConstHGShift4_Value");
    ...
    rcode =
        Fg_setParameterWithType(fg, Idxoff0, &xoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idxlen0, &x_pre_len, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode =
        Fg_setParameterWithType(fg, Idyoff0, &yoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idylen0, &height, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;

    rcode =
        Fg_setParameterWithType(fg, Idxoff1, &xoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idxlen1, &x_pre_len, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode =
        Fg_setParameterWithType(fg, Idyoff1, &yoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idylen1, &height, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;

    rcode =
        Fg_setParameterWithType(fg, Idxoff2, &xoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idxlen2, &x_pre_len, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode =
        Fg_setParameterWithType(fg, Idyoff2, &yoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idylen2, &height, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;

    rcode =
        Fg_setParameterWithType(fg, Idxoff3, &xoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idxlen3, &x_pre_len, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode =
        Fg_setParameterWithType(fg, Idyoff3, &yoff, 0, FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    rcode = Fg_setParameterWithType(fg, Idylen3, &height, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode) nError++;
    ...
    height;
    rcode = Fg_setParameterWithType(fg, Idsplitter, &height, 0,
                                    FG_PARAM_TYPE_UINT32_T);
    if (rcode)
      nError++;
    else
    ...
    /*============ HG/LG Mult  ============== */
    Value = 128;  //*32 << 2 = 128
    Fg_setParameterWithType(fg, ConstLGShift4_Value_Id, &Value, 0,
                            FG_PARAM_TYPE_INT32_T);
    if (rcode)
      nError++;
    else {
      sprintf(mesx,
              "siSoftware::initialize: global shutter: LG Mult %d (%.02f)",
              Value, (float)(Value / 4.0));
      lf.log(mesx);
    }

    Value = 13;  //*(32/10)=3.2 << 2 = 13 -> 3.25
    Fg_setParameterWithType(fg, ConstHGShift4_Value_Id, &Value, 0,
                            FG_PARAM_TYPE_INT32_T);
    if (rcode)
      nError++;
    else {
      sprintf(mesx,
              "siSoftware::initialize: global shutter: HG Mult %d (%.02f)",
              Value, (float)(Value / 4.0));
      lf.log(mesx);
    }
    /*
      if(act_sccmos_version>=HAP_VERSION_NOISE)
      {
       ...
       DoubleShutter_AppendNumber_Id =
      Fg_getParameterIdByName(fg,"Device1_Process0_DoubleShutter_AppendNumber");

       rcode = Fg_setParameterWithType(fg,DoubleShutter_AppendNumber_Id, &Value,
      0, FG_PARAM_TYPE_UINT32_T);
       if(rcode)
        nError++;
       else
       {
        sprintf(mesx,"siSoftware::initialize: global shutter: Append Images set
      to %d ",Value);
        lf.log(mesx);
        }
      }//if(act_sccmos_version>=HAP_VERSION_NOISE)
      ...
      {
       sprintf(mesx,"siSoftware::initialize: global shutter: doublemode not
      possible with this version",Value);
       lf.log(mesx);
    ...
    if (nError) {
      sprintf(mesx,
              "siSoftware::initialize: global shutter: Error because some "
              "FG_setParameter failed Errcount %d",
              nError);
      lf.log(mesx);
      return FALSE;
    }
  ...
  if (!is_force_size) {
    ...
    // leave height alone... we /2 because we have 10tap 8 bit data, to convert
    // to 16bit pixels.
  ...
  sprintf(mesx, "To Allocate %d buffers for total of %d bytes", num_buffers,
          totalBufferSize);
  lf.log(mesx);

  //!!fudgespace to make sure mem is a bit larger than images...?
  int fudge_space = 65536 * num_buffers;
  totalBufferSize += fudge_space;
...
bool siSoftware::CreateObjects() {
  //        if (is_force_size)
  //        {
  //            m_Acq->SetParameter(CORACQ_PRM_CROP_HEIGHT ,
  //            sensor_height,false);
...
void siSoftware::grab() {
  ...
  if (is_blocking == false) {
    if (Fg_AcquireEx(fg, camPort, GRAB_INFINITE, ACQ_STANDARD,
                     (dma_mem *)ptrMem) < 0) {
    ...
    if (Fg_AcquireEx(fg, camPort, GRAB_INFINITE, ACQ_BLOCK, (dma_mem *)ptrMem) <
        0) {
      sprintf(mesx, "siSoftware::grab Fg_Acquire() failed: %s\n",
              Fg_getLastErrorDescription(fg));
      lf.log(mesx);
