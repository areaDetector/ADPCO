...
tiff_file::tiff_file() {
  for (int i = 0; i < header_size; i++) head_data[i] = 0;
...
// fp = fopen(filename,'r','n');
  ...
  ReadFile(fp, (unsigned char*)&header, sizeof(tiff_header), &got, 0);
#else
  fp = fopen(filename, "r");
  if (fp == 0) throw ccd_exception("tiff_file::tifCheck bad file handle");
...
void tiff_file::tifIFD(char* filename, unsigned long offset) {
  ...
  if (is_tiff) {
    ...
    SetFilePointer(fp, (long)dir_start, 0, FILE_BEGIN);

    // fields = fread(fp,1,'ushort');

    ReadFile(fp, (unsigned char*)&num_ifd_fields, sizeof(unsigned short), &got,
             0);

    ifdlen = num_ifd_fields * 12;

    //        ifd = fread(fp,fields*12,'uchar');
    ReadFile(fp, (unsigned char*)ifd, ifdlen, &got, 0);

    //        nextifd = fread(fp,1,'ul');
    ReadFile(fp, (unsigned char*)&next_ifd, sizeof(unsigned long), &got, 0);

    CloseHandle(fp);
#else
    fopen(filename, "r");
    if (fp == 0) throw ccd_exception("tiff_file::tifIFD bad file handle");
    ...
    for (int k = 0; k < 16; k++) scalars[k] = ifd[k + 11].valoffs;
...
unsigned long tiff_file::tifFindIFD(void) { return (header.first_ifd); }
///////////////////////////////////////////////////////////
//
//
//
...
void tiff_file::tifRd(char* filename, unsigned short* img) {
  ...
  SetFilePointer(fp, strt, 0, FILE_BEGIN);

  ReadFile(fp, (unsigned char*)img, sz, &got, 0);
...
void tiff_file::tifRdInfo(char* filename) {
  ...
  if (ifd[11].tag == 6000) {
    ...
    ReadFile(fp, (unsigned char*)head_data, sz, &got, 0);
...
//
////////////////////////////////////////////////////////
void tiff_file::tifWr(char* filename, unsigned short* img, int sizex,
                      int sizey) {
  unsigned long got;
  ...
  WriteFile(fp, &header, sizeof(tiff_header), &got, NULL);

  // write image
  WriteFile(fp, (unsigned char*)img, specs.strip_bytecnt, &got, NULL);
  // write header
  WriteFile(fp, (unsigned char*)head_data, header_size, &got, NULL);
  ...
  WriteFile(fp, (unsigned char*)&next_ifd, sizeof(long), &got, NULL);
...
//
////////////////////////////////////////////////////////
void tiff_file::tifWr(HANDLE fp, unsigned short* img, int sizex, int sizey) {
  unsigned long got;
...
// write TIFF header
  ...
  WriteFile(fp, (unsigned char*)img, specs.strip_bytecnt, &got, NULL);
  // write header
  WriteFile(fp, (unsigned char*)head_data, 4096, &got, NULL);
...
void tiff_file::tifWriteIFD(void) {
  ...
  // write num of foelds
  is_ok = WriteFile(fp, (unsigned char*)&num_ifd_fields, sizeof(unsigned short),
                    &got, NULL);

  // write ifd

  is_ok = WriteFile(fp, (unsigned char*)ifd, num_ifd_fields * sizeof(ifd_field),
                    &got, NULL);
#else
  // write num of foelds
...
char* tiff_file::getHeader(void) { return head_data; }
