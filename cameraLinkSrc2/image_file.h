...
class image_file {
 public:
  ...
  // make file object.
  image_file(char* name, int count);

  image_file(char* name, int count, bool is_tiff);

  image_file(char* name, char* path, int count, bool is_tiff);
  ...
  // write data.
  bool write(char* ptr, int size);
  // write data- leaving room for header
  bool write(char* ptr, int size, int skipbytes);

  // write data.- take large img memory, write 1 quqdrant of it.UNSTICH img
  bool write(char* ptr, int size, int quadrant, int linesize);

  // set file to beginning.
  bool rewind(void);
  // read file data.
  bool read(char* ptr, unsigned long size);
  // read file data. with header skip
  bool read(char* ptr, int size, int skip);

  // read file data. with header skip- reads small img as one quadrant into
  // larger imagemem.
  // STICH IMG
  bool read(char* ptr, int size, int skip, int quadrant, int linesize);

  // make dtdosplay header wih image size.
  char* dtHeader(int x, int y);
