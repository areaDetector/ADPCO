 ...
 ******************************************************************************/
// make file object.
image_file::image_file(char *name, int count) : tiff() {
  path_name[0] = 0;
...
// make file object.
#if 1
image_file::image_file(char *name, int count, bool is_tiff) : tiff() {
  path_name[0] = 0;
...
// make file object.
#if 1
image_file::image_file(char *name, char *path, int count, bool is_tiff)
    : tiff() {
  path_name[0] = 0;
...
bool image_file::write(image_ram &ram) {
  ...
  if (is_use_tiff) {
    tiff.tifWr(fp, (unsigned short *)ram.getMem(), ram.getImgX(),
               ram.getImgY());
    return true;
  }
...
//    ww=fwrite(ptr,s,sizeof(short),fp);
  ...
  err = WriteFile(fp, ptr, s * sizeof(short), &ww, NULL);
 ...
 ******************************************************************************/
// write data.
bool image_file::write(char *ptr, int size) {
  unsigned long ww;
  bool err;
...
#ifdef _WINDOWS
  err = WriteFile(fp, ptr, size, &ww, NULL);
 ...
 ******************************************************************************/
// write data.
bool image_file::write(char *ptr, int size, int skipbytes) {
  unsigned long ww;
  bool err;
  ...
  err = WriteFile(fp, head_data, skipbytes, &ww, NULL);
 ...
 ******************************************************************************/
// write data.
bool image_file::write(char *ptr, int size, int quadrant, int linesize) {
  bool is_ok;
  unsigned long got;
  char *startptr;
  int linecnt;
  ...
  {
    startptr =
        ptr + sizeof(unsigned short) * (2 * linesize * linesize + linesize);
  }
 ...
 ******************************************************************************/
// read file data.
bool image_file::read(char *ptr, unsigned long size) {
  bool is_ok;
  unsigned long got;
...
#ifdef _WINDOWS
  is_ok = ReadFile(fp, ptr, size, &got, 0);
 ...
 ******************************************************************************/
// read file data.
bool image_file::read(char *ptr, int size, int skip) {
  bool is_ok;
  unsigned long got;
...
// read in header into image mem-- we overrite it below
  ...
  is_ok = ReadFile(fp, this->head_data, skip, &got, 0);
 ...
 * we store one quadrant of data on the disk, and then we read it into fill 2x2
 *image
 ...
 *linesize is num of pixels in the line of one quadrant- or 1/2 of total 4 quad
 *img.
 ...
 ******************************************************************************/
// read file data.
bool image_file::read(char *ptr, int size, int skip, int quadrant,
                      int linesize) {
  bool is_ok = true;
  unsigned long got;
  char *startptr;
  ...
  is_ok = this->read((char *)temp_ram->getMem(),
                     sizeof(short) * linesize * linesize);
...
char *image_file::dtHeader(int x, int y) {
 ...
 ******************************************************************************/
// make dtdosplay header wih image size.
void image_file::setTiff(bool usetiff) { is_use_tiff = usetiff; }

char *image_file::getHeader(void) {
  if (is_use_tiff) return (tiff.head_data);
