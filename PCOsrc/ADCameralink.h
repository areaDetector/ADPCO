...
class ADCameralink : public ADDriver {
 public:
  ...
  void getImageTask(int arg); /**< This should be private but is called from C,
                                 must be public */
...
#define FIRST_ADCameralink_PARAM ADCameralink_first
  ...
  int cor_check_timestamps;  // causes check of time stamps. otherwize ignore
                             // time stamps
  ...
  int cor_est_buffers_left;  // mamory left counted in buffgers; before we run
                             // out of mem
  int cor_nd_datasize;  // size in bytes of nd array
  int cor_num_repeat_timestamp;  // counters for missed and repeated time
                                 // stamps; from corecoticks
  int cor_num_missed_timestamp;
  int cor_missed_ts_wait;    // time between shots where we dont count as missed
                             // shot- say if we stop and start acq
  int cor_use_image_mode;    // causes grabber to not grab more then ADNumImages
                             // if in multi mode; or single mode. otheruse grab
                             // all shots
  int cor_reset_buff_index;  // reset buffer index to clear untransfered
                             // frames...
...
#define NUM_ADCameralink_PARAMS \
  (&LAST_ADCameralink_PARAM - &FIRST_ADCameralink_PARAM + 1)
