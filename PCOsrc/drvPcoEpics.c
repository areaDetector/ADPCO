/* drvPCOEpics.c
 *
 * This is the EPICS dependent code for the driver for a simulated area
 * detector.
...
static void blockPCOCallFunc(const iocshArgBuf *args) {
  if (strcmp(args[0].sval, "blockN") == 0) pco_blocking = args[1].ival;

  if (strcmp(args[0].sval, "prMutex") == 0) pco_pr_mutex = args[1].ival;

  if (strcmp(args[0].sval, "prMutex2") == 0) pco_pr_mutex2 = args[1].ival;
}
