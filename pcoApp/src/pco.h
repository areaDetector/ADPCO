/*! \mainpage PCO areaDetector Driver
 *
...
class epicsShareClass pco : public ADCameralink {
 public:
  ...
  void
  pcoTask(); /**< This should be private but is called from C, must be public */
  void pcoTask2(); /**< This should be private but is called from C, must be
                      public */

  epicsEventId
      stopEventId; /**< This should be private but is accessed from C, must be
                      public */
...
#define FIRST_pco_PARAM pco_first
  ...
  int pco_dbg_serread;
  int pco_dbg_serstr;
  int pco_dbg_serprint;  // set to int value of serial code to pco- then
                         // printing turns on for that code pnly

  int pco_ser_waitms;  // wait between read and write in serial transaction.
                       // coreco need none. sisw need a few ms
  int pco_do_camcontrol;  // 1 to run camera contol loop- so we can turn it off
                          // in debigg
  ...
  // dimax needs extra mem for images. if we need 1 img, we need mem for 3
  // according
...
#define LAST_pco_PARAM pco_last
  ...
  // after we get image from card, we call this. It does nothing, but can be
  // overridden
  virtual void processNewImage(void *img_ptr);
  virtual void keepHouse(void);
  int updateParameters(asynUser *pasynUser, epicsInt32 ivalue,
                       epicsFloat64 dvalue, int paramtype);

  virtual int doSerialTransactions(int function, int ivalue, double dvalue,
                                   int paramtype);
  ...
  int doSerialCommand(pco_command &cmd, pco_response &rsp, unsigned char *obj,
                      int len);
  ...
  // avoids race condition- allows camera stats to shut down ADACquire setting
  // to 0, if no recording is happening.
