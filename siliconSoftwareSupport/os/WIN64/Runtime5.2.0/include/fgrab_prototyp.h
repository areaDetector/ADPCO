...
/**
 * \brief	Initialize a frame grabber in trampoline mode.
 * \anchor  Fg_InitTrampoline
 * 
 * This function initializes a framegrabber without any applet. On me4 boards this provides basic
 * access to firmware registers. On me5 boards this can be used for full access to the board
 * configuration.
 * The BoardIndex parameter is used to address multiple framegrabbers in the same system.
 *
 * Board numbers depend on the system architecture and operating system. In general the board numbers
 * depend on the PCI setup, i.e. boards with lower PCI bus and slot numbers will have lower board
 * numbers. On Windows systems meIII grabbers will always get lower numbers than meIV devices. On
 ...
 * shows board type, serial number, and board index.
 *
 * The FPGA on the board will not be configured.
 *
 * An initialized frame grabber resource by Fg_InitTrampoline() must be released by Fg_FreeGrabber().
 *
 * \param	BoardIndex	Logical number of the board.
 *
 * \returns	Initialized pointer on the data structure of the FrameFrabber.
 ...
 * If this function returns NULL you may call Fg_getLastErrorNumber(NULL) to get the error code.
 */
Fg_Struct *Fg_InitTrampoline(unsigned int BoardIndex);
